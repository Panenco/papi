...
import { getMetadataSchema } from '../decorators/JSONSchema.decorator.js';
 ...
 */
export function validationMetadatasToSchemas(userOptions?: Partial<IOptions>): Record<string, SchemaObject> {
  ...
  const flatMetadatas = ([] as ValidationMetadata[]).concat(...metadatas.values());
...
export function validationMetadataArrayToSchemas(
  ...
  Object.entries(
    _groupBy(
      metadatas,
      ({ target }) => target[options.schemaNameField as keyof typeof target] ?? (target as Function).name,
    ),
    ...
    Object.entries(_groupBy(metas, 'propertyName')).forEach(([propName, propMetas]) => {
 ...
 */
export function targetConstructorToSchema(targetConstructor: Function, userOptions?: Partial<IOptions>): SchemaObject {
  const options: IOptions = {
    ...defaultOptions,
    ...userOptions,
  };
...
function populateMetadatasWithConstraints(storage: IStorage, metadatas: ValidationMetadata[]): ValidationMetadata[] {
  const constraints: Map<any, ConstraintMetadata[]> = (storage as any).constraintMetadatas;
 ...
 */
function getInheritedMetadatas(target: Function, metadatas: ValidationMetadata[]) {
 ...
 */
function applyConverters(propertyMetadatas: ValidationMetadata[], options: IOptions): SchemaObject {
...
function isExcluded(propertyMetadata: ValidationMetadata, options: IOptions): boolean {
 ...
 */
function getRequiredPropNames(target: Function, metadatas: ValidationMetadata[], options: IOptions) {
